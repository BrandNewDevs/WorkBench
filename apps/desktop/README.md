...
`dev` builds the Electron processes, waits for Vite to answer on `127.0.0.1:5173`, then starts Electron with the development renderer enabled. To run the built renderer without packaging the app, run `pnpm --filter @workbench/desktop build` followed by `pnpm --filter @workbench/desktop start`.

...
Login, session restoration, sign-out, and the managed IPC chat pipe are complete. The renderer loads chat threads and persisted history through the compatible `/chat/sessions` read surface. On send, Electron uploads only explicitly selected local files to `/sessions/{id}/uploads`, passes the returned IDs to `POST /sessions/{id}/messages`, treats the HTTP 202 response as queued work, and consumes `/sessions/{id}/events` through a cancellable main-process stream bridge. The UI reflects queued, processing, completed, failed, and approval-required states; Phase 4 stops at pending approval. Local paths never reach the renderer, and retry attempts preserve `clientMessageId`. Development-bypass mode remains fixture-only; sending is disabled there because no authenticated local session exists.
